...
describe("codex conversation binding", () => {
  ...
  it("clears the Codex app-server binding when a pending bind is denied", async () => {
    const transcriptLocator = path.join(tempDir, "session.jsonl");
    await seedCodexBinding(transcriptLocator, { threadId: "thread-1" });
      ...
      },
      {
        ...
        pluginBinding: {
          ...
          data: {
            kind: "codex-app-server-session",
            version: 1,
            transcriptLocator: path.join(tempDir, "session.jsonl"),
            workspaceDir: tempDir,
          },
        },
      },
  ...
  it("recreates a missing bound thread and preserves auth plus turn overrides", async () => {
    ...
    const savedBinding = await readCodexAppServerBinding(transcriptLocator);
