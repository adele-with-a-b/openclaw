...
describe("codex conversation controls", () => {
  ...
  it("persists fast mode and permissions for later bound turns", async () => {
    ...
    await expect(setCodexConversationFastMode({ transcriptLocator, enabled: true })).resolves.toBe(
      "Codex fast mode enabled.",
    );
    await expect(
      setCodexConversationPermissions({ transcriptLocator, mode: "default" }),
  ...
  it("does not persist public OpenAI provider after model changes on native auth bindings", async () => {
    ...
    await expect(setCodexConversationModel({ transcriptLocator, model: "gpt-5.5" })).resolves.toBe(
