...
export function readCodexConversationActiveTurn(
  identity: string | { sessionKey?: string; transcriptLocator?: string },
...
async function requireThreadBinding(identity: { sessionKey?: string; transcriptLocator?: string }) {
...
function resolveCodexConversationControlKey(
  ...
  return identity.sessionKey?.trim() || identity.transcriptLocator?.trim() || "";
}
